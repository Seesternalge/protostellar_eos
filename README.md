...
These four quantities are all tabulated as a function of temperature and density; by default from log10(T_min [K]) = 0.5 to log10(T_max [K]) = 6.0 in steps of log(DeltaT [K]) = 0.01 and from log10(Rho [g cm^(-3)]) = -22.0 to log10(Rho [g cm^(-3)]) = 1.0 in steps of log10(DeltaRho [g cm^(-3)]) = 0.05.

Made public in the hope that it will be useful to people working in this field. If any parts of this code or the data are used for new work, I would ask you to cite the three works listed in the first paragraph. For questions, please write to amayer@mpa-garching.mpg.de .
